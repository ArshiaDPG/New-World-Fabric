...
public class AutomatonEntity extends HostileEntity {
    ...
    public void tickMovement() {
        if (!this.isRusted() && this.method_48926().getDifficulty() == Difficulty.PEACEFUL){
            deactivate(this);
        }
        super.tickMovement();
    }
    ...
    public void onDeath(DamageSource damageSource) {
        if (isRusted()){
            ...
            AutomatonEntity automatonEntity = new AutomatonEntity(NWEntityTypes.AUTOMATON, this.method_48926());
            deactivate(automatonEntity);

            this.remove(RemovalReason.DISCARDED);
            this.method_48926().spawnEntity(automatonEntity);
        }
    }
